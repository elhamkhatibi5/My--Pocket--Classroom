import { loadCapsule, loadCapsules, loadProgress, saveProgress } from './storage.js';
import { escapeHTML } from './utils.js';

export function initLearnTab() {
  const learnTab = document.querySelector('[data-view="learn"]');
  learnTab.addEventListener('click', e => {
    e.preventDefault();
    const capsules = loadCapsules();
    const container = document.getElementById('learn');
    if (!capsules.length) {
      container.innerHTML = '<p class="text-center mt-3">No capsules to learn. Create one in Library!</p>';
      return;
    }
    // نمایش اولین Capsule پیش‌فرض
    renderLearn(capsules[0].id);
  });
}
...
export function renderLearn(id) {
  ...
  if (!capsule) {
    container.innerHTML = '<div class="alert alert-danger">Capsule not found</div>';
    return;
  }
  ...
  container.innerHTML = `
    <div class="mb-3 d-flex justify-content-between align-items-start flex-wrap">
      <div>
        <h3 class="mb-0"><i class="bi bi-book-half"></i> ${escapeHTML(capsule.meta?.title || capsule.title)}</h3>
        <div class="small text-muted">
          ${escapeHTML(capsule.meta?.subject || 'General')} • 
          <span class="badge bg-info">${escapeHTML(capsule.meta?.level || 'Basic')}</span>
        </div>
      </div>
      <div class="text-end small text-muted">Updated: ${escapeHTML(capsule.updatedAt || '')}</div>
    </div>

    <div class="btn-group mb-3" role="group">
      <button class="btn btn-outline-primary active" data-mode="notes"><i class="bi bi-journal-text"></i> Notes</button>
      <button class="btn btn-outline-primary" data-mode="flashcards"><i class="bi bi-card-list"></i> Flashcards</button>
      <button class="btn btn-outline-primary" data-mode="quiz"><i class="bi bi-question-circle"></i> Quiz</button>
    </div>
  ...
  function showNotes() {
    if (!capsule.notes || capsule.notes.length === 0) {
      body.innerHTML = '<p>No notes available.</p>';
      return;
    }
    body.innerHTML = `
    ...
    body.querySelector('#note-search').addEventListener('input', e => {
      ...
      body.querySelectorAll('#note-list li').forEach(li =>
  ...
  function showFlashcards() {
    if (!capsule.flashcards || capsule.flashcards.length === 0) {
      body.innerHTML = '<p>No flashcards available.</p>';
      return;
    }
    let idx = 0;
    ...
    function renderCard() {
      ...
      body.innerHTML = `
        <div class="card p-3 text-center shadow-sm">
          ...
          <p class="mt-2 small text-muted">Card ${idx + 1} of ${capsule.flashcards.length}</p>
        </div>
      `;
      ...
      // کیبورد: Space برای Flip
      document.onkeydown = (e) => {
        if (e.code === 'Space' && mode === 'flashcards') {
          e.preventDefault();
  ...
  function showQuiz() {
    if (!capsule.quiz || capsule.quiz.length === 0) {
      body.innerHTML = '<p>No quiz available.</p>';
      return;
    }
    let idx = 0, score = 0;
    const progress = loadProgress(capsule.id) || {};
    ...
    function renderQuestion() {
      ...
      body.innerHTML = `
        <div class="card p-3 shadow-sm">
          ...
          <div class="list-group mb-2">
            ${q.choices.map((c, i) =>
              `<button class="list-group-item list-group-item-action q-choice" data-idx="${i}">${escapeHTML(c)}</button>`
            ).join('')}
          </div>
          <div id="feedback" class="mb-2"></div>
          <p class="small text-muted">Question ${idx + 1} of ${capsule.quiz.length}</p>
        </div>
      `;
      body.querySelectorAll('.q-choice').forEach(btn => btn.addEventListener('click', () => {
        ...
        if (chosen === correct) {
          ...
          feedback.innerHTML = `<div class="text-danger">Wrong. Correct: ${escapeHTML(q.choices[correct])}</div>`;
        }
        setTimeout(() => {
          idx++;
